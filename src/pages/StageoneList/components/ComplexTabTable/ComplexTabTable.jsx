...
export default class ComplexTabTable extends Component {
  ...
  componentWillMount() {
    ...
    const result = ajaxTo('api.php?entry=sys&c=teacher&a=student&do=groupList',{username:userInfo.admin});
...
onClose = () => {
  ...
  deleteId = () => {
    ...
    console.log(id);
    ajaxTo('api.php?entry=sys&c=teacher&a=student&do=del', {'id': id,username:that.props.user.admin}).then((res)=>{
      console.log(res);
  ...
  renderOperations = (value, index, record) => {
    const toUrl = '/groupchecklist/' + record.id;
    const toUrl1='/StatusChange/'+record.id;
    const toUrl2='/stageone/'+record.id;
    const toUrl3='/preparation/'+record.id;
    const toCode='testcode/'+record.id;
    const datapic='datapic/'+record.id;
    const realtimedata='realtimedata/'+record.id;
    const reviewScore = '/statistics/'+record.id

    const exportUrl = baseURL + 'api.php?entry=sys&c=group&a=export&do=export_stage1&group_id='+record.id
    ...
    return (<div style={styles.complexTabTableOperation}>
      ...
      <Link style={aStyle} to={reviewScore}>成绩查看</Link>
    </div>);
  };
  ...
  render() {
    ...
    const userInfo=this.state.userInfo.admin;
    console.log(userInfo);
        ...
        <IcePanel style={{
          ...
          <IcePanel.Body>
            ...
            <Table dataSource={tableData.data} isLoading={tableData.__loading} className="basic-table" style={styles.basicTable} hasBorder={false} onRowClick={onRowClick}>
              <Table.Column title="分组名称" width={60} dataIndex="gname"/>
              <Table.Column className={userInfo == 'admin'?'block':'none'} title="小组老师" width={60} dataIndex="teacher"/>
              <Table.Column title="所属阶段" width={60} dataIndex="stage"/>
              <Table.Column title="创建日期" style={{}} width={100} dataIndex="createtime"/>

              <Table.Column title="当前状态" dataIndex="status" width={60} cell={this.renderStatus}/>
