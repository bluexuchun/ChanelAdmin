...
export default class CreateHaveNumForm extends Component {
  field = new Field(this);
  static displayName = 'CreateHaveNumForm';
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      currentTab: 'solved',
      currentCategory: '1',
      ajaxUrl: 'api.php?entry=sys&c=browse&a=browse&do=update'
    };
  }
  //获取题库列表
  componentWillMount() {
    ...
    const that = this;
    const result = ajaxTo('api.php?entry=sys&c=browse&a=browse&do=list', {sid: this.props.history.params.id});
    result.then(function(res) {
      if (res.data) {
        console.log(res.data.data)

        that.setState({allData: res.data.data});
      }
    })
  }
  //题库
  ...
  deleteId = (id) => {
    console.log(id);
    ajaxTo('api.php?entry=sys&c=browse&a=browse&do=del', {'id': id});
  ...
  editorNum(id) {
    console.log('编辑顺序');
    const that = this;
    ajaxTo('api.php?entry=sys&c=browse&a=browse&do=edit', {'id': id}).then((res) => {
      console.log(res)
      ...
      let items = [];
      currentData.product.map((item, index) => {
        items.push({name: item.name, judge: item.judge, detail: item.detail})
      })
      console.log(items)
      that.setState({
        value: {
  ...
  onTabChange(key) {
    this.reset();
    this.setState({currentKey: key, btnword: '立即创建', ajaxUrl: 'api.php?entry=sys&c=browse&a=browse&do=update'});
    console.log('切换tab顺序');
  }
  submit = () => {
    console.log(this.props);
    ...
    setTimeout(function() {
      // that.props.history.router.push('/activityList');
      const result = ajaxTo('api.php?entry=sys&c=browse&a=browse&do=list', {sid: that.props.history.params.id});
      result.then(function(res) {
        console.log(res)

        that.setState({allData: res.data.data, currentKey: '1'});
      })
      // that.setState()
    ...
    return (<div className="create-activity-form">
      ...
      render() {
        ...
        return (<div style={{
          ...
          <Button type='primary' style={styles.formItem1} onClick={this.props.addItem}>添加选项</Button>
                ...
                return (<Row key={index} style={{
                  ...
                  <Col>
                    ...
                    <IceFormBinder name={`items[${index}].detail`}>
                      <Input multiple="multiple" name={`items[${index}].detail`} placeholder=""/>
                    </IceFormBinder>
